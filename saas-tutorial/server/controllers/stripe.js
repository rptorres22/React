    ...
    stripe.events.retrieve(receivedEvent, function(err, verifiedEvent) {
      ...
      switch(verifiedEvent.type) {
        ...
        case "invoice.payment_succeeded":
          User.findOne({ customerId: verifiedEvent.data.object.customer }, function(err, user) {
            if (err) { return next(err); }
...
exports.createSubscription = function(req, res, next) {
  ...
  User.findById(req.user._id, function(err, user) {
    ...
    stripe.customers.create({
      ...
      let amtOfSubs = customer.subscriptions.total_count;
      user.stripe.lastFour = req.body.lastFour;
...
exports.changeSubscription = function(req, res, next) {
    ...
    User.findById(req.user._id, function(err, userToChange) {
      if (err) { return next(err); }

      stripe.subscriptions.update(
        userToChange.stripe.subscriptionId,
        { plan: req.body.newPlan }).then(function(subscription) {
          ...
          userToChange.save(function(err) {
            if (err) {
              res.status(422).send({ error: 'There was an error processing your request.' });
              return next(err);
            }
...
exports.getCustomer = function(req, res, next) {
  User.findById(req.user._id, function(err, userToFetch) {
    if (err) {
      return next(err);
    }
